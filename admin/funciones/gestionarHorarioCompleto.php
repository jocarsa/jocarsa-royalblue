...
function gestionarHorarioCompleto($pdo) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $pdo->exec("UPDATE hourly_availability SET available=0");
        ...
        if (isset($_POST['availability']) && is_array($_POST['availability'])) {
            foreach ($_POST['availability'] as $dayOfWeek => $hoursArr) {
                foreach ($hoursArr as $hour => $val) {
                    if ($val == "1") {
                        $pdo->prepare("UPDATE hourly_availability SET available=1 WHERE day_of_week=:d AND hour=:h")
                            ->execute([':d'=>$dayOfWeek,':h'=>$hour]);
                    }
                }
            }
        }
    ...
    $disp = [];
    $res = $pdo->query("SELECT day_of_week, hour, available FROM hourly_availability");
